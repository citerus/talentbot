import os
from flask import Flask
from flask import request
from talents import TrelloTalents
from talentbot import TalentCommand, SlackEvent
from command import Help, FindPeopleByTalent, FindTalentsByPerson

# Set these variables in your local environment
# When deploying to Heroku, these variables can
# be set with the heroku-keys.sh script.

apiKey      = os.environ['TRELLO_API_KEY']
apiSecret   = os.environ['TRELLO_API_SECRET']
tr_token    = os.environ['TRELLO_TOKEN']
tokenSecret = os.environ['TRELLO_TOKEN_SECRET']
token       = os.environ['SLACK_TOKEN']

app = Flask(__name__)

@app.route('/')
def hello():
    request_token = request.args.get('token')
    if (request_token != token):
        # TODO Possibly return 401?
        return "No!"
    
    request_text = request.args.get('text')
    trello = TrelloTalents(api_key=apiKey, api_secret=apiSecret, token=tr_token, token_secret=tokenSecret)
    
    talentCommand = TalentCommand()
    talentCommand.addCommand(Help())
    talentCommand.addCommand(FindTalentsByPerson(trello))
    talentCommand.addCommand(FindPeopleByTalent(trello))
    
    event = SlackEvent(request)
    
    return talentCommand.processEvent(event)

# In order to get proper debugging support while
# developing locally, start the server using 'python main.py'
if __name__ == "__main__":
     app.run(debug=True)
